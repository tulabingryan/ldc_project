...
from CREATOR import df_houseSpecs, df_hvacSpecs, df_heaterSpecs, df_fridgeSpecs, df_freezerSpecs 
...
from CREATOR import df_pvSpecs, df_windSpecs
...
class HeatPump(Load):
    def __init__(self):
        ...
        for key in list(df_hvacSpecs):
            self.params[key] = df_hvacSpecs.loc[self.n, key] 

        self.setup()


class ElectricHeater(Load):
    def __init__(self):
        Load.__init__(self)
        ...
        for key in list(df_pvSpecs):
            self.params[key] = df_pvSpecs.loc[self.n, key]
...
def add_unit(device, load_type, n_unit=0, n_ldc=0, idx=10, report=False):
    ...
    dict_load_specs = {
        ...
        'hvac': [df_hvacSpecs, device.App.__dict__['df_baseload']],
...
def make_devices(dict_devices, 
    ...
    df_hvacSpecs = pd.read_csv('./specs/hvacSpecs.csv')
...
if __name__ == '__main__':
    ...
    for i in range(60*60*36):
        ...
        x = device.step()
        print(device.App.isotime, t1-t0, device.App.a_status)
        print(x)
