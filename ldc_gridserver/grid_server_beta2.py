...
global dict_cmd, dict_agg, cmd_algorithm, cmd_loading, previous_limit, ldc_signal, latest_demand, df_data, df_data_cols, display_limit, df_data_cols
display_limit =True
dict_agg = {}
dict_cmd = {}
...
def get_data(dict_msg={"timescale":1, "unixstart":0, "unixend":0},
    db_name='./ldc_agg_flat.db', duration=60*10, mcast_ip="224.0.2.3", mcast_port=16003, report=False):
    """ Fetch data from the local database"""
    global dict_cmd, dict_agg, cmd_algorithm, cmd_loading, previous_limit, ldc_signal, latest_demand, df_data, df_data_cols, display_limit, df_data_cols
    dict_cmd.update(dict_msg)
    # send_command(dict_cmd=dict_cmd, ip=tcp_ip, port=tcp_port)

    start = dict_msg['unixstart']
    end = dict_msg['unixend']

    con = lite.connect(db_name)
    con.execute('pragma journal_mode=wal;')
    row = con.execute('select * from data limit 1')
    col_labels = [desc[0] for desc in row.description]
    ...
    while True and len(data) < 1:
        try:
            ...
            with con:
                ...
                data = np.array(cur.fetchall())
                df_data = pd.DataFrame(data, columns=col_labels)
                df_data = df_data.drop(labels=["localtime"], axis=1)
                df_data = df_data.astype(float)
                col_labels.remove('localtime')
...
#     global ser, latest_demand, cmd_loading, cmd_algorithm, ldc_signal, previous_limit, df_data_cols
#     # define timeout
...
def send_command(dict_cmd, ip='192.168.1.81', port=10000):
    # try:
...
sum_actual = 0
sum_proposed = 0
df_data = get_data(dict_msg={"timescale":1, "unixstart":0, "unixend":0})
...
def update_loading(n_submit, n_blur, algorithm, loading):
    ...
    dict_cmd.update({"agg_limit":str(cmd_loading), "algorithm":str(cmd_algorithm), "frequency":str(ldc_signal), "cmd":msg})
    send_command(dict_cmd=dict_cmd, ip=tcp_ip, port=tcp_port)
...
def update_gain(n_submit, n_blur, gain):
...
def update_signal(n_submit, n_blur, n_intervals, value):
...
def update_data(algorithm, loading, n_intervals, json_data):
    # update the graph
    global dict_cmd, dict_agg, cmd_algorithm, cmd_loading, previous_limit, ldc_signal, latest_demand, df_data, df_data_cols, display_limit, df_data_cols
    ...
    try:  # get old data and append new data
        try:
            ...
            unixstart = int(df_data_old.tail(1)['unixtime'])
        except Exception as e:
        ...
        try:
            while len(df_data_new.index) <= 0:
                df_data_new = get_data(dict_msg={"algorithm":cmd_algorithm, "loading":cmd_loading, "frequency":ldc_signal, "timescale":1, "unixstart":unixstart, "unixend":0})
        ...
        try:
            ...
            df_data = df_data.groupby('unixtime', as_index=False).mean().tail(60*10).reset_index(drop=True)
            # print(df_data.tail(10))
...
def update_graph(json_data):  
    ...
    trace_actual = go.Scattergl(
                ...
                y = df_data_graph['agg_demand'] * 0.001,  # agg_demand
                name = 'Demand',
    ...
    trace_power1 = go.Scattergl(
                x = df_data_graph.index,
                y = df_data_graph['power_active_1'],
                name = 'Phase 1',
                line= {'color':'rgb(255,0,0)'},
                # opacity = 0.8,
                # fill = "tozeroy",
    ...
    trace_power2 = go.Scattergl(
                x = df_data_graph.index,
                y = df_data_graph['power_active_2'],
                name = 'Phase 2',
                line= {'color':'rgb(0,255,0)'},
                # opacity = 0.8,
                # fill = "tozeroy",
    ...
    trace_power3 = go.Scattergl(
                x = df_data_graph.index,
                y = df_data_graph['power_active_3'],
                name = 'Phase 3',
