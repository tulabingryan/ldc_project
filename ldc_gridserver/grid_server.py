...
def get_data(day, unixstart=None, unixend=None):
  ...
  while len(df_data.index)<=0:
    try:
      df_data = pd.read_pickle(f'history/{day}.pkl')
    except Exception as e:
      df_data = pd.read_feather(f'history/{day}.feather')
      pass
...
def send_command(dict_cmd, ip='localhost', port=10000, report=True, timeout=0.5):
  try:
    response = MULTICAST.send(dict_cmd, ip="224.0.2.3", port=16003, timeout=timeout)
    if report:
      print('sent:', dict_cmd)
...
df_data = get_data(day=datetime.datetime.now().strftime('%Y-%m-%d')).reset_index(drop=True)
...
@app.callback(
  dash.dependencies.Output('hidden-target', 'value'),
...
@app.callback(
  dash.dependencies.Output('settings', 'children'),
  [
  dash.dependencies.Input('input-target-kw', 'n_submit'),
  dash.dependencies.Input('cmd-algorithm','value'),
  dash.dependencies.Input('cmd-set-target','value'),
  # dash.dependencies.Input('btn_cmd_drop', 'n_clicks_timestamp'),
  dash.dependencies.Input('input-target-kw', 'value')
  ],
  [])
...
@app.callback(
  dash.dependencies.Output('input-target-kw', 'disabled'),
  [dash.dependencies.Input('cmd-set-target','value')],
  [])
...
@app.callback(
  dash.dependencies.Output('hidden-gain', 'children'),
...
@app.callback(
  dash.dependencies.Output('hidden-freq', 'children'),
  [dash.dependencies.Input('input-cmd-freq', 'n_submit'),
  dash.dependencies.Input('input-cmd-freq', 'value')],
  [])
...
@app.callback(
  dash.dependencies.Output('display-target-kw', 'children'),
...
def update_display_target(n_intervals):
  ...
  d = send_command(dict_cmd={'states':'target_watt'}, report=False, timeout=0.1)
  if d:
    cmd_target_watt = d["target_watt"]
    return f'{np.round(cmd_target_watt*1e-3, 2)}'


@app.callback(
  dash.dependencies.Output('display-signal-hz', 'children'),
  [dash.dependencies.Input('periodic-target-update', 'n_intervals')],
  )
def update_display_signal(n_intervals):
  # change the frequency signal 
  global refresh_rate, gain, dict_cmd, dict_agg, cmd_algorithm, cmd_target_watt, ldc_signal, latest_demand
  d = send_command(dict_cmd={'states':'signal'}, report=False, timeout=0.1)
  if d:
    ldc_signal = d['signal']
    return f'{np.round(ldc_signal, 2)}'

@app.callback(
  dash.dependencies.Output('display-gain', 'children'),
  [dash.dependencies.Input('periodic-target-update', 'n_intervals')],
  )
def update_display_gain(n_intervals):
  # change the frequency signal 
  global refresh_rate, gain, dict_cmd, dict_agg, cmd_algorithm, cmd_target_watt, ldc_signal, latest_demand
  d = send_command(dict_cmd={'states':'gain'}, report=False, timeout=0.1)
  if d: 
    gain = d['gain']
        ...
        html.Div([
          ...
          html.H1("Microgrid History", style={'marginTop':'5', 'text-align':'center','float':'center', 'color':'white'}),
           ...
           html.Div([
            ...
            html.Div([
                html.Div(id='display-target-kw', children=np.round(float(cmd_target_watt)/1000, 3), className='row', 
