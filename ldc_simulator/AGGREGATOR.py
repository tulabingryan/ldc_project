  ...
  def autorun(self):
    ...
    while True:
      try:
        ...
        [p.send(self.dict_common) for p in self.ready_loads]
        [p.send(self.dict_common) for p in self.common_observer]

        ### gather data from the device simulators
        self.ready_loads = multiprocessing.connection.wait(self.load_pipes, timeout=0)
        for p in self.ready_loads:
          new = p.recv()
          k = new['load_type'][0]
        ...
        if self.simulation==1:
          ...
          if self.target=='auto': 
            alpha = 1-np.exp(-self.dict_common['step_size']/86400)
            self.target_loading = (self.dict_common['loading_percent']*alpha) + (self.target_loading*(1-alpha)) 
