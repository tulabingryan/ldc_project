...
class EnergyMeter():
    ...
    def read_meter(self, param='power_active'):
        ...
        for _id in self.powermeter_ids:
            ...
            dict_data[f'{param}_{_id}'] = value * factor

        return dict_data


    def save_data(self, df_demand):
        try:
    ...
    def autorun(self):
        ...
        while True:
            try:
                self.get_meter_data(params=['power_active', 'voltage', 'current'], report=True)
                time.sleep(0.5)
...
if __name__ == '__main__':
    EM1 = EnergyMeter(house='HHH_000', IDs=[0])
    while True:
        try:
