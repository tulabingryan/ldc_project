...
def clock(unixtime, realtime=True, step_size=1):
  ...
  if realtime:
    ...
    step_size = np.random.uniform(0.01, step_size*1.5)
...
####### models for ldc #################
def ldc_injector(ldc_signal, latest_watt, target_watt, step_size, algorithm, hour=0, minute=0, 
  previous_error=0, previous_i_term=0, K=5, ):
  '''
  ...
  if algorithm=='no_ldc':
    ...
    error = np.subtract(target_watt, latest_watt) #np.divide(np.subtract(target_loading, latest_loading), target_loading)
    delta_error = np.subtract(error, previous_error)
    ...
    d_term = np.divide(delta_error, step_size)
    new_signal = np.add(p_term, np.add(np.multiply(Ki, i_term), np.multiply(Kd, d_term))) 
    return {'ldc_signal': np.clip(new_signal, a_min=0, a_max=100),
...
def read_signal(old_signal, new_signal, n_units, resolution=1, delay=0.0, step_size=1, simulation=0):
  ...
  try:
    ...
    delay = np.clip(np.random.normal(delay, 1e-6, n_units), a_min=1e-6, a_max=delay)
...
def normalize(value, a_min=20, a_max=80, expected_max=100):
...
def ldc_dongle(states, common):
  ...
  try:
    ...
    if common['algorithm']=='no_ldc':
      ...
      if (states['load_type'][0] in ['heater', 'waterheater']): ### change the proposed status to avoid thermostat lock up
        revised_cooling = ((((states['temp_in']>=np.subtract(states['temp_target'], np.multiply(states['tolerance'], 0.8))) & (states['actual_status']==1)) 
                      | ((states['temp_in']>=np.add(states['temp_target'], np.multiply(states['tolerance'], -0.5)))&(states['actual_status']==0)))&(states['mode']==0)) * 1
        revised_heating = ((((states['temp_in']<=np.add(states['temp_target'], np.multiply(states['tolerance'], 0.8)))&(states['actual_status']==1)) 
                      |  ((states['temp_in']<=np.subtract(states['temp_target'], np.multiply(states['tolerance'], -0.5)))&(states['actual_status']==0)))&(states['mode']==1)) * 1
        # revised_cooling = ((((states['temp_in']>=np.subtract(states['temp_target'], np.multiply(states['tolerance'], 0.5)))))&(states['mode']==0)) * 1
        # revised_heating = ((((states['temp_in']<=np.add(states['temp_target'], np.multiply(states['tolerance'], 0.5)))))&(states['mode']==1)) * 1
      ...
      if common['algorithm']=='basic_ldc':
        ### change priority
        if (common['ranking']=='dynamic')&(int(common['unixtime'])%60==0): 
        ...
        flex_priority = normalize(states['flexibility'] * 100, a_min=20, a_max=80)
        uniform_priority = np.random.uniform(20, 80, len(states['priority']))
        ...
        priority = ((w)*states['priority']) + ((1-w)*new_priority)

      ### change actual_status
      if states['load_type'][0] in ['clothesdryer', 'clotheswasher', 'dishwasher']:
        partial_status = np.multiply(proposed_status, ((states['priority']<=ldc_signal)|np.invert(states['with_dr'])|(states['flexibility']<=0))*1)
        ...
        if common['tcl_control'] in ['setpoint', 'mixed']:
          actual_status = np.multiply(1, ((states['priority']<=ldc_signal)|np.invert(states['with_dr']))*1)
          ...
          actual_status = np.multiply(proposed_status, ((states['priority']<=ldc_signal)|np.invert(states['with_dr'])|(states['flexibility']<=0))*1)
    ...
    ready_to_change = np.flatnonzero(np.subtract(states['counter'], states['min_cycletime'])>0)  # index of counters > min_cycletime
    changed = np.flatnonzero(np.add(states['actual_status'][ready_to_change], actual_status[ready_to_change])==1)  # index of those that actually changed, xor
    ...
    states['counter'][ready_to_change[changed]] = np.subtract(states['counter'][ready_to_change[changed]], states['min_cycletime'][ready_to_change[changed]]) + np.random.uniform(-30, 0, len(changed))
    actions.update({'priority': priority, 'actual_status':states['actual_status'], 'ldc_signal':ldc_signal, 'old_signal':dict_signal['old_signal'], 'counter':states['counter']})
