...
class VRM_API:
    ...
    def graph_widgets(self, inst_id, measurement_codes, instance=None, start=None, end=None):
        ...
        if start and end:
            data_dict['start'] = int(start)
            data_dict['end'] = int(end)
...
if __name__ == '__main__':
  api = VRM_API(username='rtul220@aucklanduni.ac.nz', password='ArdmoreECE098')
  site_data = api.get_user_sites(api.user_id)
  dict_labels = {
    'Gc':'grid_to_consumer', 
    'Gb':'grid_to_battery',
    'Pc':'pv_to_consumer',
    'Pb':'pv_to_battery',
    'Bc':'battery_to_consumer',
    'Bg':'batter_to_grid'
    }
  while True:
    try:
      for rec in site_data['records']:
        unixtime = time.time()
        data = api.get_consumption_stats(inst_id=rec['idSite'], start=unixtime-(3600*24), end=unixtime )
        if data['success'] and rec['name']=='ArdmoreHouse4Y':
          for p in ['Gc', 'Pc', 'Pb', 'Bc']:
            if p in data['records'].keys():
              df = pd.DataFrame(np.array(data['records'][p]).reshape((-1,2)))
              df.columns = ['unixtime', dict_labels[p]]
              df.index = pd.to_datetime(df['unixtime'], unit='ms')
