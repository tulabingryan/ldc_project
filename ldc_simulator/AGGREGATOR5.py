...
class Aggregator(object):
  ...
  def __init__(self, dict_devices, timestamp, latitude, longitude, idx, local_ip, device_ip, step_size=1, 
    ...
    resolution=3, ki=5, save_history=True, tcl_control='direct', delay=0.0, 
    report=False, flex_percent=100):
    ...
    self.dict_common.update({'local_ip':local_ip, 'unixtime': self.timestamp, 'previous_error':0, 'previous_i_term':0, 'tcl_control':tcl_control,
      'case':self.case, 'algorithm':self.algorithm, 'ranking':self.ranking, 'distribution':self.distribution, 'delay':delay,
      'resolution':self.resolution, 'target':target, 'target_percent':self.target_loading, 'simulation':simulation, 
      'ldc_signal':0, 'flex':flex_percent/100})
  ...
  def autorun(self):
    ...
    if self.simulation==1:
      ...
      # if self.save_history: self.list_processes.append(multiprocessing.Process(target=self.history, args=()))
      ...
      self.list_processes.append(threading.Thread(target=self.udp_server, args=()))
      self.list_processes.append(threading.Thread(target=self.multicast_server, args=()))
      self.list_processes.append(threading.Thread(target=self.ldc_listener, args=()))
      self.common_observer.extend([self.pipe_agg_listener0, self.pipe_agg_multicast0, self.pipe_agg_udp0])
    ...
    while True:
      try:
        ...
        for k in self.loads_to_run:
          ...
          new = eval(f'self.pipe_agg_{k}0.recv()')
          self.dict_summary_demand[f'{k}'] = [new['actual_demand'][new['house']==h].sum() for h in self.dict_house['name']]
        ...
        if self.simulation==1:
          ...
          #   if len(dict_history[k].keys())>self.save_interval:
          #     self.pipe_agg_history0.send({'states':dict_history, 'common':self.dict_common.copy()})
          ...
          if self.pipe_agg_network0.poll():
            ...
            pfe_mw = (0.9*pfe_mw) + (0.1*(grid_data['loss_percent'] - (nogrid_percent*factor)))
          ...
          print(self.dict_common['isotime'], 
            ...
            'target_kva:', np.round(self.target_loading*sn_mva*10,3), 
            'latest_kva:', np.round(self.dict_common['loading_percent']*sn_mva*10,3), 
  ...
  def ldc_listener(self):
    ...
    try:
      ...
      spi_address = f'{ip[0]}.{ip[1]}.{ip[2]}.101'
      while True:
        try:
          ...
          if (s>0.0):
            ...
            try:
              response = MULTICAST.send(dict_msg={"injector":"ldc_signal_spi"}, ip=spi_address, port=10000, timeout=0.1, hops=1)
              self.dict_injector.update({'ldc_signal':np.array([response[spi_address]['ldc_signal_spi']])})
            except Exception as e:
              print(f'Error AGGREGATOR.ldc_listener.query_signal_with_spi:{e}')
              response = MULTICAST.send(dict_msg={"injector":"ldc_signal_spi"}, ip='224.0.2.0', port=17000, timeout=0.1, hops=1)
              for k in response.keys():
                if 'ldc_signal_spi' in response[k].keys():
                  self.dict_injector.update({'ldc_signal':np.array([response[spi_address]['ldc_signal_spi']])})
                  spi_address = k
          self.pipe_agg_listener1.send(self.dict_injector)
          time.sleep(self.pause)
      ...
      while True:
        try:
          ...
          try:
            ...
            print(f'Error AGGREGATOR.ldc_listener.query_signal_no_spi:{e}')
            response = MULTICAST.send(dict_msg={"injector":"ldc_signal_spi"}, ip='224.0.2.0', port=17000, timeout=0.1, hops=1)
            for k in response.keys():
              if 'ldc_signal_spi' in response[k].keys():
                self.dict_injector.update({'ldc_signal':np.array([response[spi_address]['ldc_signal_spi']])})
    ...
    while True:
      try:
        ...
        if self.case.startswith('ramp') or self.case.startswith('ideal'):
          ...
          self.dict_baseload['actual_demand'] = np.array([df.loc[x, y] for x, y in zip(sk.astype(int), self.dict_baseload['schedule'])]) + np.abs(np.random.normal(0,1))
        ### send update to main
        ...
        ### save data
        if self.simulation==1 and self.save_history:
          # dict_save.update({int(self.dict_common['unixtime']):self.dict_baseload.copy()})
  ...
  def heatpump(self):
    ...
    while True:
      try:
        ...
        # ### update ldc_signal  # done withing ldc_dongle
        ...
        if self.simulation==0:
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_heatpump.copy()})
    ...
    while True:
      try:
        ...
        if self.simulation==0:
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_heater.copy()})
  ...
  def waterheater(self):
    ...
    ip = self.local_ip.split('.')
    sensor_address = f'{ip[0]}.{ip[1]}.{ip[2]}.113'
    ...
    while True:
      try:
        ...
        if self.simulation==0:
          ### get actual readings
          try:
            response = MULTICAST.send(dict_msg={"pcsensor":"temp_in"}, ip=sensor_address, port=10000, timeout=0.1, hops=1)
            self.dict_waterheater['temp_in'][0] = response[sensor_address]['temp_in']
            time.sleep(1)
          except Exception as e:
            print("Error AGGREGATOR.waterheater.get_temp_in:", e)
            response = MULTICAST.send(dict_msg={"pcsensor":"temp_in"}, ip='224.0.2.0', port=17000, timeout=0.1, hops=1)
            for k in response.keys():
              if 'temp_in' in response[k].keys():
                self.dict_waterheater['temp_in'][0] = response[keys[0]]['temp_in']
                sensor_address = k
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_waterheater.copy()})
    ...
    while True:
      try:
        ...
        if self.simulation==0:
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_fridge.copy()})
    ...
    while True:
      try:
        ...
        if self.simulation==0:
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_freezer.copy()})
    ...
    while True:
      try:
        ...
        if self.simulation==0:
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_clotheswasher.copy()})
    ...
    while True:
      try:
        ...
        if self.simulation==0:
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_clothesdryer.copy()})
    ...
    while True:
      try:
        ...
        if self.simulation==0:
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_dishwasher.copy()})
    ...
    while True:
      try:
        ...
        if self.simulation==0:
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_ev.copy()})
    ...
    while True:
      try:
        ...
        if self.simulation==0:
          ...
          ### save data
          if self.save_history:
            # dict_save.update({int(self.dict_common['unixtime']):self.dict_storage.copy()})
  ...
  def solar(self):
    ...
    while True:
      try:
        ...
        ### save data
        if self.simulation==1 and self.save_history:
          # dict_save.update({int(self.dict_common['unixtime']):self.dict_solar.copy()})
  ...
  def wind(self):
    ...
    while True:
      try:
        ...
        ### save data
        if self.simulation==1 and self.save_history:
          # dict_save.update({int(self.dict_common['unixtime']):self.dict_wind.copy()})
      ...
      while True:
        try:
          ...
          try:
            ...
            for k in d.keys(): 
              self.dict_summary_demand.update(d[k])
          except Exception as e:
            print(f'Error AGGREGATOR.drive_grainy.query_demand:{e}')
          ### add all demand
          total = 0.0
    ...
    while True:
      try:
        try:
          ...
          print("Error in multicast_server binding socket:",e)
          print("Retrying...")
          time.sleep(10)
        except KeyboardInterrupt:
          raise KeyboardInterrupt
        ...
        while True:
          ### receive data from main routine
          self.dict_common = self.pipe_agg_multicast1.recv()
          if self.dict_common['is_alive']==False:
            raise KeyboardInterrupt 
          ...
          if msg_in:
            try:
              ...
              ### note: sock.sendto is used in each case to avoid sending empty replies
              if ('states' in rcv_keys) and ('states' in agg_keys):
                if self.dict_agg['states']:
              ...
              if ('summary' in rcv_keys) and ('summary' in agg_keys):
                if self.dict_agg['summary'][dict_msg['summary']]:
              ...
              if 'injector' in rcv_keys:
                if dict_msg['injector'] in self.dict_injector.keys():
                  if self.dict_injector[dict_msg['injector']]:
              ...
              print("Error in multicast_server respond loop:", e, dict_msg)
            except KeyboardInterrupt:
  ...
  def udp_server(self):
    ...
    while True:
      try:
        try:
          udp_address_port = (self.local_ip, 17001)
          sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)
          sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
          ...
          mreq = struct.pack('4sL', group, socket.INADDR_ANY)
          sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP,mreq)
          print(f'H{self.house_num} udp_server:', udp_address_port)
        except Exception as e:
          print("Error in udp_server binding socket:",e)
          print("Retrying...")
          time.sleep(10)
        except KeyboardInterrupt:
          raise KeyboardInterrupt
        ...
        while True:
          ...
          if msg_in:
            try:
              ...
              print("Error in udp_server respond loop:", e, dict_msg)
            except KeyboardInterrupt:
              self.pipe_agg_comm1.close()
              sock.close()
              raise KeyboardInterrupt
      ...
      try:
        ...
        for k in dict_agg['states'].keys():
          ...
          if (len(dict_save[k].keys())>=self.save_interval) and (self.case!=None):
            dict_save[k] = save_data(dict_agg['states'][k], case=self.case,  folder=self.casefolder, filename=f'{k}.h5')
        ...
        for k in dict_agg['states'].keys():
          save_data(dict_agg['states'][k], case=self.case,  folder=self.casefolder, filename=f'{k}.h5')
