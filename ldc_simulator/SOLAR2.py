...
@jit(nopython=True)
def get_climate_type(latitude, is_summer):
    ...
    climate_type = ((latitude>23.5)&(latitude<=66.5)&(is_summer==1)) * 1
    climate_type = np.add(climate_type, (((latitude>23.5)&(latitude<=66.5)&(is_summer==0)) * 3))
    climate_type = np.add(climate_type, ((latitude>66.5)* 2))
    return climate_type
...
def get_solar_time(**kwargs):
    ...
    try:
        ...
        kwargs['longitude'][idx1] = np.subtract(360, kwargs['longitude'][idx1])
        kwargs['longitude'][idx2] = np.abs(kwargs['longitude'][idx2])

        kwargs['minute_diff'] = np.array((4*(kwargs['meridian']-kwargs['longitude']))+E).astype('timedelta64[m]')  # time difference in minutes
...
def get_cleaness_index(**kwargs):
    ...
    try:
        ...
        print("Error solar get_cleaness_index:", e)
    
    return kwargs

### test
# dict_vars = get_cleaness_index(**dict_vars)
...
if __name__ == '__main__':
    ...
    for dt in dt_range:
        dict_vars.update({'unixtime':dt.timestamp()})
        dict_vars.update(
            {
                'latitude': np.array([-36.86656223, -36.86680643, -36.86660351]), 
                'longitude': np.array([174.76659403, 174.76654129, 174.76667513]), 
                'elevation': np.array([29.92616073, 30.70202449, 29.41078759]), 
                'tilt': np.array([0, 30.62185867, 33.65287833]), 
                'azimuth': np.array([  180.27189771,  -99.31783374, -180.21836098]), 
                'albedo': np.array([0.19355514, 0.21359651, 0.16194601]), 
                'humidity': np.array([0.2749629 , 0.56681515, 0.36294318])
            }
        )
